...
import icu.Xell.obfuscator.util.XellStringUtil;
...
public class XellDumpState {
    ...
    public XellDumpState(OutputStream w, boolean strip, CompilerOptions opts) {
    ...
    public static int dump(Prototype f, OutputStream w, boolean strip, CompilerOptions opts) throws IOException {
        XellDumpState D = new XellDumpState(w, strip, opts);
        D.dumpHeader();
    ...
    public static int dump(Prototype f, OutputStream w, boolean stripDebug, int numberFormat, boolean littleendian, CompilerOptions opts) throws IOException {
        ...
        XellDumpState D = new XellDumpState(w, stripDebug, opts);
    ...
    void dumpString(LuaString s) throws IOException {
        String str = XellStringUtil.rot13(s.tojstring());

        int mid = str.length() / 2;
        String[] parts = {str.substring(0, mid), str.substring(mid)};

        LuaString finalString = LuaString.valueOf(parts[0] + "|XELL|" + parts[1]);
    ...
    void dumpConstants(final Prototype f) throws IOException {
        ...
        dumpInt(n + 3); //Xellvm will get rid of +3
        for (i = 0; i < n; i++) {
