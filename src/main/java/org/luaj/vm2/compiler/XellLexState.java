...
import org.luaj.vm2.*;
import org.luaj.vm2.compiler.XellFuncState.BlockCnt;
...
public class XellLexState {
    ...
    XellFuncState fs;  /* `XellFuncState' is private to the parser */
    XellLuaCompiler L;
    ...
    public XellLexState(XellLuaCompiler state, InputStream stream, CompilerOptions opts) {
        this.z = stream;
    ...
    void save(int c) {
        if (buff == null || nbuff + 1 > buff.length)
            buff = XellLuaCompiler.realloc(buff, nbuff * 2 + 1);
        buff[nbuff++] = (byte) c;
    }
    ...
    void inclinenumber() {
        int old = current;
        XellLuaCompiler._assert(currIsNewline());
        nextChar(); /* skip '\n' or '\r' */
    ...
    void setinput(XellLuaCompiler L, int firstByte, InputStream z, LuaString source) {
        this.decpoint = '.';
        this.L = L;
	...
	*/
    ...
    void read_numeral(SemInfo seminfo) {
        XellLuaCompiler._assert(isdigit(current));
        do {
            save_and_next();
...
//			trydecpoint(str, seminfo); /* try to update decimal point separator */
    ...
    void lookahead() {
        XellLuaCompiler._assert(lookahead.token == TK_EOS);
    ...
    int registerlocalvar(LuaString varname) {
        XellFuncState fs = this.fs;
        Prototype f = fs.f;
        if (f.locvars == null || fs.nlocvars + 1 > f.locvars.length)
            f.locvars = XellLuaCompiler.realloc(f.locvars, fs.nlocvars * 2 + 1);
...
//
    ...
    void new_localvar(LuaString name, int n) {
        XellFuncState fs = this.fs;
        fs.checklimit(fs.nactvar + n + 1, XellFuncState.LUAI_MAXVARS, "local variables");
    ...
    void pushclosure(XellFuncState func, expdesc v) {
        XellFuncState fs = this.fs;
        Prototype f = fs.f;
        if (f.p == null || fs.np + 1 > f.p.length)
            f.p = XellLuaCompiler.realloc(f.p, fs.np * 2 + 1);
        f.p[fs.np++] = func.f;
    ...
    void open_func(XellFuncState fs) {
        XellLuaCompiler L = this.L;
    ...
    void close_func() {
        ...
        f.code = XellLuaCompiler.realloc(f.code, fs.pc);
        f.lineinfo = XellLuaCompiler.realloc(f.lineinfo, fs.pc);
        // f.sizelineinfo = fs.pc;
        f.k = XellLuaCompiler.realloc(f.k, fs.nk);
        f.p = XellLuaCompiler.realloc(f.p, fs.np);
        f.locvars = XellLuaCompiler.realloc(f.locvars, fs.nlocvars);
        // f.sizelocvars = fs.nlocvars;
        f.upvalues = XellLuaCompiler.realloc(f.upvalues, f.nups);
        // XellLuaCompiler._assert (CheckCode.checkcode(f));
        XellLuaCompiler._assert(fs.bl == null);
        this.fs = fs.prev;
//		L.top -= 2; /* remove table and prototype from the stack */
    ...
    void constructor(expdesc t) {
        ...
        this.checknext('{');
        do {
            XellLuaCompiler._assert(cc.v.k == VVOID || cc.tostore > 0);
            if (this.t.token == '}')
                break;
        ...
        XellLuaCompiler.SETARG_B(i, luaO_int2fb(cc.na)); /* set initial array size */
        XellLuaCompiler.SETARG_C(i, luaO_int2fb(cc.nh));  /* set initial table size */
    }
    ...
    void parlist() {
                ...
                switch (this.t.token) {
                    ...
                    case TK_DOTS: {  /* param . `...' */
                        ...
                        if (LUA_COMPAT_VARARG) {
                            ...
                            f.is_vararg = XellLuaCompiler.VARARG_HASARG | XellLuaCompiler.VARARG_NEEDSARG;
                        }
                        f.is_vararg |= XellLuaCompiler.VARARG_ISVARARG;
                        break;
                    }
                    default:
        ...
        f.numparams = (fs.nactvar - (f.is_vararg & XellLuaCompiler.VARARG_HASARG));
        fs.reserveregs(fs.nactvar);  /* reserve register for parameters */
    }

    void body(expdesc e, boolean needself, int line) {
        /* body -> `(' parlist `)' chunk END */
        XellFuncState new_fs = new XellFuncState(opts);
        open_func(new_fs);
    ...
    void funcargs(expdesc f) {
        ...
        XellLuaCompiler._assert(f.k == VNONRELOC);
        base = f.u.s.info; /* base register for call */
        if (hasmultret(args.k))
            nparams = XellLua.LUA_MULTRET; /* open call */
        else {
            if (args.k != VVOID)
    ...
    void simpleexp(expdesc v) {
        ...
        switch (this.t.token) {
            ...
            case TK_DOTS: { /* vararg */
                ...
                fs.f.is_vararg &= ~XellLuaCompiler.VARARG_NEEDSARG; /* don't need 'arg' */
    ...
    void block() {
        ...
        this.chunk();
        XellLuaCompiler._assert(bl.breaklist.i == NO_JUMP);
        fs.leaveblock();
    }
    ...
    void exprstat() {
        ...
        if (v.v.k == VCALL) /* stat -> func */
            XellLuaCompiler.SETARG_C(fs.getcodePtr(v.v), 1); /* call statement uses no results */
        ...
        if (block_follow(this.t.token) || this.t.token == ';')
            ...
            if (hasmultret(e.k)) {
                ...
                if (e.k == VCALL && nret == 1) { /* tail call? */
                    XellLuaCompiler.SET_OPCODE(fs.getcodePtr(e), opts.getOpcode("OP_TAILCALL"));
                    XellLuaCompiler._assert(XellLua.GETARG_A(fs.getcode(e)) == fs.nactvar);
                }
                first = fs.nactvar;
                nret = XellLua.LUA_MULTRET; /* return all values */
            } else {
                if (nret == 1) /* only one single value? */
                    ...
                    XellLuaCompiler._assert(nret == fs.freereg - first);
                }
            }
        }
        fs.ret(first, nret);
    ...
    void chunk() {
        ...
        while (!islast && !block_follow(this.t.token)) {
            ...
            this.testnext(';');
            XellLuaCompiler._assert(this.fs.f.maxstacksize >= this.fs.freereg
